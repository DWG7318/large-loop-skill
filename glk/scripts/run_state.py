...
def _graph_edge(value):
    ...
    for label, item in (("source", source), ("target", target), ("justification", justification)):
...
def recompute_graph_topology(baseline, fallback_go_ids=()):
    ...
    if explicit:
        ...
        terminal_go_ids = _sorted_texts(
            _value(baseline, "terminal_go_ids"), "GRAPH_TERMINAL_SET_INVALID", "terminal_go_ids"
        )
        required_go_ids = _sorted_texts(
            _value(baseline, "required_go_ids"), "GRAPH_REQUIRED_GO_COVERAGE_INVALID", "required_go_ids"
        )
...
def _maximum_compatible_go_ids(nodes):
    ...
    def search(index, chosen, used_keys):
        ...
        if index == len(candidates):
            ...
            if len(chosen_ids) > len(best) or (len(chosen_ids) == len(best) and chosen_ids < best):
                best = chosen_ids
            return
