...
GLK is the Graph-based Loop Engineering method for one large engineering Run. It expresses the Run as a multi-start directed acyclic graph (DAG) of necessary GOs and delegates every GO's linear construction to one fixed latest-SLK baseline.

```text
multiple start GOs
        -> dependency-driven GO activation
        -> one Fusion GO
        -> one final D2
        -> completed Run
```
...
- One Graph is one Run; every node is one numerically named GO such as `GO001`.
- GOs start as soon as all direct prerequisites exist. Every join uses `ALL`; unrelated work does not wait.
- One Supervisor owns graph design, member setup, routing, Fusion, final D2, and archival.
- Every GO has an exclusive Checker/Worker pair that follows SLK. GLK does not duplicate SLK's CELL, D0, D1, rework, communication, record, or model guidance.
- Normal construction ends in one Fusion GO. A prebuilt conditional D2 Repair GO is used only when final D2 fails.
- `$slk-select-models` is the single model-selection authority for SLK, CLK, and GLK.

## Run artifacts

- `GLK-GRAPH.md`: Supervisor-owned DAG and GO-contract authority; all members may read.
...
GLK 3.2.0 contains four sibling Skill directories:
...
| `skills/glk-run-graph/SKILL.md` | Member setup, GO activation, handoff routing, and unstarted-region amendments |
