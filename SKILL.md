...
1. Freeze objective, scope, acceptance, resources, write boundaries, hard brakes, and route rules before execution.
2. Create fresh role contexts for every unrelated project. Worker/Checker and Checker/Router must be different contexts on the same Loop.
3. Every CELL requires a Worker validation receipt and independent Checker validation for the same artifact identity.
4. Only Router answers `Goal verified?` and chooses `done`, `continue`, `replan`, `reject`, or `manual-intervention`.
5. Checker verifies but cannot close. Worker completion is not acceptance. Supervisor and Owner cannot replace Router.
6. Planner chooses CELLs and may propose splitting a coarse Loop, but cannot implement, accept, route, or silently mutate a frozen contract.
7. Worker executes only a formal CELL or rework assignment and never chooses the next CELL or broadens scope.
8. Evidence and authority records are append-only. Corrections supersede; they never erase.
9. Silence, timeouts, partial files, stale green tests, or an unavailable role never imply success.
10. `manual-intervention` is a non-terminal hold, never closure, merge permission, or release permission.
11. Runtime learning may inform a future contract but cannot mutate the active one.
12. LLK output is a candidate handoff and never silently overwrites authoritative project assets.
...
2. Keep the current task as Supervisor. Create fresh Planner, Worker, Checker, and Router contexts when agent tools exist; record IDs and bindings.
3. Read [references/ROLE_CONTRACTS.md](references/ROLE_CONTRACTS.md) before role launch and [references/CONTRACT_AND_STATE.md](references/CONTRACT_AND_STATE.md) before creating the `.large-loop/` control tree. Never store credentials or raw secrets.
4. Planner writes the project plan, initial Loop contract, CELL strategy, resource bindings, and acceptance mapping.
...
One cycle is exactly one ordered `Planner -> Worker -> Checker -> Router` pass. Only the active role acts. Other roles may observe but cannot inject competing decisions.
...
- Lost role context: bind a fresh role through recorded authority transfer and provide the verified recovery packet.

Never auto-advance. Resume from the last valid append-only record. See [references/EVIDENCE_AND_RECOVERY.md](references/EVIDENCE_AND_RECOVERY.md).
...
Every red flag means stop, preserve evidence, restore the correct authority, and resume through a formal record.
